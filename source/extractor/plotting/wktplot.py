...
class WKTPlot:
    ...
    def __init__(self, title: str, save_dir: Union[str, Path, None] = None):
        ...
        if save_dir is not None:
            ...
            filename = save_dir / f"{_remove_symbols(title)}.html"
            output_file(filename=filename, title=title, mode="inline")

        self.figure = figure(title=title, x_axis_label="x", y_axis_label="y", match_aspect=True)
...
class WKTLayout:
    ...
    def __init__(self, title: str, save_dir: Union[str, Path], grid_size: tuple[int, int]):
        if not title:
            title = _get_random_string()
            self.logger.info(f"Given title is empty, setting title to [{title}]")
        ...
        self.grid: list[list[Union[Figure, None]]] = [[None] * grid_size[1]] * grid_size[0]

    def add_figure(self, figure: Union[WKTPlot, Figure], position: tuple[int, int]):

        f = figure if isinstance(figure, Figure) else figure.figure
...
def _get_random_string(string_length: int = 6) -> str:
